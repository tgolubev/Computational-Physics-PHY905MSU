...
[File,Path]=uigetfile('*.txt','MultiSelect','off');
 ...
 steps_per_orbit = round((5*87.969/365.25)/time_step)   %1 orbit of mercury is 88 days, use round to round to  the nearest integer of steps
...
%  initial_perihelion_angle = atand(abs(y_initial_perihelion/x_initial_perihelion))   %verify that initial perihelion angle is 0 
 ...
 x_final_perihelion = data(mercury_indices(perihelion_index),4)
 y_final_perihelion = data(mercury_indices(perihelion_index),5)
 z_final_perihelion = data(mercury_indices(perihelion_index),6)
