...
%numeric_soln = zeros(1,N);                %initialize arrays to store the numeric and analytic solutions for each file. This initialization is not necessary but makes code faster.
%analytic_soln = zeros(1,N);
...
numeric_soln_shifted(end,1) = 0; 
 ...
 set(h,'LineWidth',1.5);                              %make lines thicker
...
hold off               %so doesn't add curves from different runs
