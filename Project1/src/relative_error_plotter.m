...
%For each plot line you want, need to run the code again. It has 'hold on'
%enabled, so eachnew generated line will be added to the currently opened
%figure.

clear                                                       %to clear variables
...
for  num=1:N                                                  %Repeat loop for each file (each single curve analyzed seperately)
   ...
   max_error(num,1) = max(rel_error); %finds the maximum error in the column for that specific iteration and puts it into row = num(corresponding to which iteration/file its from)
   n = size(rel_error,1)+1; %From thenumber of rows (1st dimension of rel_error: synthax is size(variable, dimension) where for 
...
%  average = mean(log_y_data,2);
%  
%  figure;
%  plot(x_data,average)
end

 set(0,'defaulttextinterpreter','tex')  %Sets .tex format as the default text interpreter. This displays i.e. 'log_{10}' with 10 being a subscript on labels. Use i.e. 'words^{superscript}'
                                        %to make a superscript.

 g = plot(log_h, max_error);
 set(g,'Color','b')
 set(g,'LineWidth',1.5);                              %make lines thicker
 hold on                                              %here we purposely leave hold on, so can run this code again to generate another line
                                                      %which will be added
                                                      %to the same opened
                                                      %figure.
