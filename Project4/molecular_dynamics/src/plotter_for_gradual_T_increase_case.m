...
[File,Path]=uigetfile('*.txt','MultiSelect','off');
 ...
 %of the graph before have phase transition.
  ...
  clearvars temp_Rsquared                                                          %clear VB_Rsquared matrix b/c otherwise might have leftover rows from previous file/curve (which had different bandedge position)
  clearvars m
...
%  end
 ...
 %l = ezplot(temp_fitline, [0,time(transition_index)]);
